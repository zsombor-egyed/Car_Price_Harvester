...
for (car in cars){
  print(car)
  property_table <- tryCatch(read.csv(paste0("./binded_tables/", car), stringsAsFactors = FALSE), error = function(e){"NA"})
  if (property_table != "Missing"){
...
for (car in cars){
  i <- i + 1
  print(i)
  property_table <- tryCatch(read.csv(paste0("./binded_tables/", car), stringsAsFactors = FALSE), error = function(e) "Missing")
  if (property_table != "Missing"){
